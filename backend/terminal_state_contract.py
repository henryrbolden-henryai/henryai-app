...
# =============================================================================
# P1-CRITICAL: PM EXPERIENCE DETECTION HELPERS
# =============================================================================
...
def _count_pm_experience(resume: dict) -> Tuple[int, int, int]:
    ...
    for role in resume.get("experience", []):
        ...
        is_internship = any(re.search(p, title) for p in internship_patterns)
        if not is_internship:
            is_internship = any(re.search(p, company) for p in internship_patterns)

        if is_internship:
            pm_internship_count += 1
        else:
            # Try to estimate duration for full-time roles
            # Default to 24 months if we can't determine
...
def detect_terminal_state(
    ...
    # P1-CRITICAL: Check for internship-only PM experience
    ...
    # P1-CRITICAL: Override detected level for internship-only PM experience
